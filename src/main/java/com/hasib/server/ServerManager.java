...
public class ServerManager implements MessageListener {
    ...
    public ServerManager() {
        client = new Clients[ServerConstant.CLIENT_NUMBER];
        clientTracker = new String[ServerConstant.CLIENT_NUMBER];
        serverExeCutor = Executors.newCachedThreadPool();
    }

    public void startServer(ServerStatusListener statusListener, ClientListener clientListener) {
        try {
            statusListener.status("Server is Listening on port : " + ServerConstant.SERVER_PORT);

            server = new ServerSocket(ServerConstant.SERVER_PORT, ServerConstant.BACKLOG);
    ...
    public void controllConnection(ServerStatusListener statusListener, ClientListener clientListener) {
        while (clientNumber < ServerConstant.CLIENT_NUMBER) {
            try {
